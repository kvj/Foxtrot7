...
yepnope({
    load: ['lib/jquery.js', 'bs/css/bootstrap.min.css', 'bs/js/bootstrap.min.js', 'lib/date.js', 'lib/cross-utils.js', 'res/main.css'],
    complete: function () {
...
var Native = function() { // Provides interface to native functions like bt, network, notifications etc.
};

Native.prototype.getBTState = function(handler) { // Returns adapter state
    chrome.bluetooth.getAdapterState(function(state) { // Received state
        log('Received state', state);
    }.bind(this));
    chrome.bluetooth.getDevices({
        deviceCallback: function(device) { // When found
            log('Device found: ', device);
        }.bind(this)
    }, function(state) { // Received state
        log('getDevice end', state);
    }.bind(this));
};
...
var App = function() {
    ...
    this.initDB(function(err) {
        ...
        log('App started, searching for devices');
        this.os = new Native();
        this.os.getBTState(function(err) { // Called when state is received
        }.bind(this));
